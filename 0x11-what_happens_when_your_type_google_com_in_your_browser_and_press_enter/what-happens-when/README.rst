...
* If not found, the browser calls ``gethostbyname`` library function (varies by
  OS) to do the lookup.
* if the ip of the requested page is not present in the ISP'S DNS server, the servers send
  a request to the authorized DNS server(canme dns value) and gets the requested ip 
